...
async fn test_setup_totp_generates_secret_and_qr() {
    ...
    let user = auth_svc
        .register("totp1@example.com", "password123", None)
        .await
        .unwrap();

    let (secret, qr) = mfa_svc
        .setup_totp(&user.id.to_string(), "TestIssuer")
        .await
        .unwrap();
...
async fn test_verify_totp_setup_wrong_code() {
    ...
    let mfa_svc = MfaService::new(state.db.clone(), state.config.clone());
    let user = auth_svc
        .register("totp2@example.com", "password123", None)
        .await
        .unwrap();
    ...
    mfa_svc
        ...
        .unwrap();
    let result = mfa_svc
        .verify_totp_setup(&user.id.to_string(), "000000")
        .await;
...
async fn test_verify_totp_wrong_code() {
    ...
    let mfa_svc = MfaService::new(state.db.clone(), state.config.clone());
    let user = auth_svc
        .register("totp3@example.com", "password123", None)
        .await
        .unwrap();

    let (secret, _) = mfa_svc
        .setup_totp(&user.id.to_string(), "Test")
        .await
        .unwrap();
    ...
    let valid = mfa_svc
        .verify_totp(&user.id.to_string(), "000000")
        .await
        .unwrap();
    assert!(!valid);
}
...
async fn test_disable_totp_wrong_code() {
    ...
    let mfa_svc = MfaService::new(state.db.clone(), state.config.clone());
    let user = auth_svc
        .register("totp4@example.com", "password123", None)
        .await
        .unwrap();
...
async fn test_regenerate_backup_codes_count() {
    ...
    let mfa_svc = MfaService::new(state.db.clone(), state.config.clone());
    let user = auth_svc
        .register("totp5@example.com", "password123", None)
        .await
        .unwrap();
    ...
    let codes = mfa_svc
        .regenerate_backup_codes(&user.id.to_string())
        .await
        .unwrap();
...
async fn test_get_backup_codes_count_no_totp() {
    ...
    let mfa_svc = MfaService::new(state.db.clone(), state.config.clone());
    let user = auth_svc
        .register("totp6@example.com", "password123", None)
        .await
        .unwrap();
