...
async fn test_register_duplicate_email() {
    ...
    svc.register("dup@example.com", "password123", None).await.unwrap();
...
async fn test_login_inactive_user() {
    let state = setup_db().await;
    let svc = AuthService::new(state.db.clone(), state.config.clone());
    let user = svc.register("inactive@example.com", "password123", None).await.unwrap();
...
async fn test_access_token_roundtrip() {
    let state = setup_db().await;
    let svc = AuthService::new(state.db.clone(), state.config.clone());
    let user = svc.register("token@example.com", "password123", None).await.unwrap();
...
async fn test_refresh_token_revoke_and_verify() {
    let state = setup_db().await;
    let svc = AuthService::new(state.db.clone(), state.config.clone());
    let user = svc.register("refresh@example.com", "password123", None).await.unwrap();
    
    let token = svc.create_refresh_token(&user.id.to_string(), None).await.unwrap();
...
async fn test_revoke_all_user_tokens() {
    let state = setup_db().await;
    let svc = AuthService::new(state.db.clone(), state.config.clone());
    let user = svc.register("revokeall@example.com", "password123", None).await.unwrap();
    
    let t1 = svc.create_refresh_token(&user.id.to_string(), None).await.unwrap();
    let t2 = svc.create_refresh_token(&user.id.to_string(), None).await.unwrap();
    
    svc.revoke_all_user_tokens(&user.id.to_string()).await.unwrap();
