...
async fn test_bounce_unknown_token() {
    ...
    let result = processor
        .process_bounce("unknown-token", "hard", "bounce")
        .await;
    assert!(result.is_ok());
}
...
async fn test_bounce_hard_threshold() {
    ...
    let processor = BounceProcessor::new(state.db.clone());
    processor
        .process_bounce(&sub.token, "hard", "bounce1")
        .await
        .unwrap();
    processor
        .process_bounce(&sub.token, "hard", "bounce2")
        .await
        .unwrap();
    ...
    processor
        .process_bounce(&sub.token, "hard", "bounce3")
        .await
        .unwrap();
...
async fn test_bounce_soft_no_unsubscribe() {
    ...
    for _ in 0..5 {
        processor
            .process_bounce(&sub.token, "soft", "soft_bounce")
            .await
            .unwrap();
    }
...
async fn test_bounce_unknown_type() {
    ...
    let processor = BounceProcessor::new(state.db.clone());
    let result = processor
        .process_bounce(&sub.token, "unknown_type", "reason")
        .await;
    assert!(result.is_ok());
}
